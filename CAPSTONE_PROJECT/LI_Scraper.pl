...
if ($name) {
	#system ('wget -q -U Mozilla -O output www.google.com/#q="linkedin"+"'. $fname .'+'. $lname .'"+\"ohio+state+university\"');
	open OUT, "<output" or die "what happen\n";
	$done = 0;
	while (<OUT> && !$done) {
		if ($_ =~ m/($fname.$lname)(.*?\n?)*(ohio state university)/) {
			system ('mv output ./profiles/'. $fname .'.'. $lname .'-LinkedIn');
			$done = 1;
		}
	}
	close OUT;
...
#else { 
system ("ls -l ./profiles/*-LinkedIn > output"); #}
open OUT, "<output" or die "what happen\n";

$tmpregex = '\d\d:\d\d ([^-]*)-';
if ($name) { $tmpregex = '\d\d:\d\d ./profiles/('. $fname .'.?'. $lname .'[^-]*)-LinkedIn'; }
...
foreach (keys %matches) {
	...
	if ($elem) {
		...
		$tmp = Regexer (0, './profiles/'. $elem ."-LinkedIn");
		if ($tmp && $tmp != 1) {
			print "$tmp\n";
			$tmp2 = 1;
		}
